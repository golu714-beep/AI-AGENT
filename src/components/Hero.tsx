...
import { motion, useMotionValue, useSpring as useFramerSpring, useTransform, AnimatePresence } from "framer-motion";
import { MessageSquare, Zap, Play, ChevronRight, Mic2, Star, Target, Sparkles, ArrowRight, Smartphone, Activity } from "lucide-react";
...
const Hero = () => {
  ...
  return (
    <section ref={containerRef} className="relative min-h-[100svh] flex flex-col items-center justify-center pt-28 md:pt-40 pb-20 md:pb-32 overflow-hidden bg-[#030303]">
      ...
      <div className="absolute top-40 left-10 md:left-20 hidden lg:flex flex-col gap-10 pointer-events-none">
          ...
          >
              <span className="text-[9px] font-mono text-gray-500 uppercase tracking-[0.3em] font-bold">System_Coord_X</span>
              <motion.span className="text-sm font-mono text-primary/80 font-black">
                  {mounted ? Math.floor(Math.random() * 1000).toString().padStart(4, '0') : "0000"}
              </motion.span>
          </motion.div>
          <motion.div 
            initial={{ opacity: 0, x: -20 }}
            ...
            className="flex flex-col gap-1"
          >
              <span className="text-[9px] font-mono text-gray-500 uppercase tracking-[0.3em] font-bold">Neural_Load</span>
              <motion.div 
      ...
      <div className="absolute bottom-40 right-10 md:right-20 hidden lg:flex flex-col gap-10 pointer-events-none text-right">
          <motion.div 
            ...
            className="flex flex-col gap-1"
          >
              <span className="text-[9px] font-mono text-gray-500 uppercase tracking-[0.3em] font-bold">Latency_Optimized</span>
              <span className="text-sm font-mono text-emerald-500/80 font-black tracking-widest uppercase">STABLE_LINK</span>
          </motion.div>
          <motion.div 
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
              <span className="text-[9px] font-mono text-gray-500 uppercase tracking-[0.3em] font-bold">Packet_Sync</span>
                  ...
                  {[...Array(5)].map((_, i) => (
                      <motion.div 
                        ...
                        animate={{ 
                          opacity: [0.1, 1, 0.1],
                          scale: [0.9, 1.1, 0.9],
                          backgroundColor: ["rgba(139,92,246,0.1)", "rgba(139,92,246,0.6)", "rgba(139,92,246,0.1)"]
                        }}
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 text-center">
        ...
        >
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.5em] text-gray-400 group-hover/badge:text-white transition-colors">Intelligence_OS_v2.0</span>
          ...
          <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-black leading-[0.8] tracking-tighter text-white inline-block">
            <div className="flex flex-wrap justify-center gap-x-[0.2em] gap-y-2 md:gap-y-4">
              {["THE", "CUSTOMER"].map((word, wordIdx) => (
                <div key={wordIdx} className="flex overflow-hidden py-2 md:py-6 whitespace-nowrap">
                  {word.split("").map((char, charIdx) => (
                    <motion.span
                      ...
                      className={cn(
                        ...
                        word === "CUSTOMER" && "text-transparent stroke-white/40 [-webkit-text-stroke:1.5px_rgba(255,255,255,0.3)] hover:[-webkit-text-stroke:1.5px_rgba(139,92,246,1)]"
                      )}
                    >
                      {char}
                    </motion.span>
        ...
        >
          <span className="text-white/60">Unified predictive voice agents and deep intelligence.</span> <br />
          Built for the modern enterprise that demands <span className="text-primary font-bold uppercase tracking-widest px-1">Total Scale.</span>
        </motion.p>
          ...
          >
            ...
            <div className="bg-[#050505] border border-white/[0.15] shadow-[20px_40px_80px_rgba(0,0,0,0.9)] overflow-hidden">
                ...
                <div className="px-4 md:px-8 py-4 md:py-5 border-b border-white/10 flex items-center justify-between bg-[#080809]">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-white/90 tracking-[0.3em] uppercase">SYSTEM_LINK_ON</span>
                    </div>
                    <div className="flex items-center gap-8">
                        <div className="hidden md:flex items-center gap-6">
                            <div className="flex flex-col items-end">
                                <span className="text-[10px] font-mono text-gray-400 uppercase tracking-widest">Signal_Source</span>
                                <span className="text-xs font-black text-primary uppercase">AZ_SAT_DATA_88</span>
                            </div>
                            <div className="flex flex-col items-end">
                                <span className="text-[10px] font-mono text-gray-400 uppercase tracking-widest">Neural_Sync</span>
            ...
            <div className="hidden xl:block absolute -left-32 top-1/2 -translate-y-1/2 space-y-10 text-right">
                <div className="flex flex-col gap-1 pr-6 border-r border-primary/40">
                    <span className="text-[10px] font-mono text-gray-500 uppercase tracking-[0.2em]">Proc_Node</span>
                    <span className="text-xs font-black text-white/90 uppercase italic tracking-widest">FF_X82A</span>
                </div>
                <div className="flex flex-col gap-1 pr-6 border-r border-primary/40">
                    <span className="text-[10px] font-mono text-gray-500 uppercase tracking-[0.2em]">Lat_Cap</span>
                    <span className="text-xs font-black text-primary uppercase italic tracking-widest">&lt; 12MS</span>
                </div>
            </div>
            ...
            <div className="hidden xl:block absolute -right-32 top-1/2 -translate-y-1/2 space-y-10 text-left">
                <div className="flex flex-col gap-1 pl-6 border-l border-white/20">
                    <span className="text-[10px] font-mono text-gray-500 uppercase tracking-[0.2em]">Auth_Level</span>
                    <span className="text-xs font-black text-white/70 uppercase tracking-widest">PREMIUM</span>
                </div>
                <div className="flex flex-col gap-1 pl-6 border-l border-white/20">
                    <span className="text-[10px] font-mono text-gray-500 uppercase tracking-[0.2em]">Conf_Index</span>
                    <span className="text-xs font-black text-white/70 uppercase tracking-widest">99.8%</span>
                </div>
            </div>
          </motion.div>
        </motion.div>
