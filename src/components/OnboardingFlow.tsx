...
import { motion, useScroll, useSpring, useTransform, useInView, AnimatePresence, useMotionValue } from "framer-motion";
import { UserPlus, Settings, Rocket, ArrowRight, Sparkles, Activity, ShieldCheck, Cpu, Layers, Terminal, Database, Zap } from "lucide-react";
...
const flowSteps = [
  {
    ...
    features: ["Biometric Verification", "Cloud Provisioning", "Permission Mapping"],
...
const StepCard = ({ step, index, total }: { step: typeof flowSteps[0], index: number, total: number }) => {
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-4">
            <div className="flex flex-col gap-1">
                <span className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">{step.label}</span>
                <span className="text-[7px] font-mono text-gray-500 uppercase">{step.serial}</span>
            </div>
            <div className="flex items-center gap-4">
                <div className="flex flex-col items-end opacity-20 group-hover/card:opacity-100 transition-opacity">
                    <span className="text-[6px] font-mono text-gray-500 uppercase">Load_Factor</span>
                    <div className="flex gap-0.5">
                        {[1,2,3,4].map(i => (
                            <div key={i} className={cn("w-1.5 h-1", i <= 3 ? "bg-primary" : "bg-white/10")} />
                        ))}
                    </div>
                </div>
        ...
        <div className="relative z-10" style={{ transform: "translateZ(50px)" }}>
          ...
          <h3 className="text-xl font-black text-white mb-2 tracking-tighter uppercase italic group-hover/card:translate-x-2 transition-transform">{step.title}</h3>
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {step.features.map((f, i) => (
              <div key={i} className="flex items-center gap-3 p-3 bg-white/[0.02] border border-white/5 hover:border-primary/30 transition-colors group/feat">
                <div className="w-1 h-4 bg-primary/20 group-hover/feat:bg-primary transition-colors" />
                <span className="text-[9px] font-black text-gray-500 group-hover/feat:text-white transition-colors uppercase tracking-widest">{f}</span>
              </div>
            ))}
          </div>
          ...
          <div className="pt-4 border-t border-white/5 flex items-center justify-between mt-auto">
              {Object.entries(step.stats).map(([key, val], i) => (
                  <div key={i} className="flex flex-col gap-1">
                      <span className="text-[7px] font-mono text-gray-500 uppercase tracking-tighter">{key}</span>
                      <span className="text-[12px] font-black text-white/40 group-hover/card:text-white transition-colors uppercase tracking-widest">{val}</span>
                  </div>
              ))}
          </div>
        </div>
...
const OnboardingFlow = () => {
  ...
  return (
    <section id="onboarding" ref={sectionRef} className="py-20 bg-[#050505] relative overflow-hidden">
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
              {mounted && [...Array(5)].map((_, i) => (
                  <motion.div
                    ...
                    transition={{ duration: 5 + Math.random() * 5, repeat: Infinity, delay: i * 2, ease: "linear" }}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col items-center text-center mb-12">
          ...
          >
            INITIALIZATION <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-purple-400 to-primary drop-shadow-[0_0_20px_rgba(139,92,246,0.3)]">PROTOCOL.</span>
          </motion.h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 relative items-stretch">
          {flowSteps.map((step, index) => (
            <StepCard key={step.id} step={step} index={index} total={flowSteps.length} />
          ))}
          
          {/* Animated Connecting Links */}
          <div className="hidden lg:block absolute inset-0 -z-10 pointer-events-none">
              <svg className="w-full h-full opacity-10">
                  <path d="M 12.5% 50% L 87.5% 50%" stroke="white" strokeWidth="1" strokeDasharray="4 4" fill="none" />
              </svg>
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-8 flex justify-center items-center gap-8 opacity-20">
              <div className="flex flex-col items-center">
                  <span className="text-[8px] font-mono text-gray-500 uppercase">Latency_Check</span>
                  <span className="text-[10px] font-black text-white uppercase italic tracking-widest">OK</span>
              </div>
              <div className="w-[1px] h-8 bg-white/10" />
              <div className="flex flex-col items-center">
                  <span className="text-[8px] font-mono text-gray-500 uppercase">Node_Availability</span>
                  <span className="text-[10px] font-black text-white uppercase italic tracking-widest">STABLE</span>
              </div>
          </div>
        </motion.div>
      </div>
