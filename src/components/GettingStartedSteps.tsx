...
const steps = [
  {
    ...
    label: "Step 01",
    description:
      "Initialize your workspace and secure your enterprise credentials.",
  ...
  },
  {
    ...
    label: "Step 03",
    description:
      "Inject proprietary data into our specialized LLM architecture.",
  ...
  },
  {
    ...
    label: "Step 04",
    description:
      "Synchronize your contact ecosystem with raw processing power.",
...
const Fragment = ({ index, color }: { index: number; color: string }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute w-32 h-32 border border-white/5 bg-white/5 rounded-xl backdrop-blur-[2px]",
        color,
      )}
      style={{
        left: `${30 + (index % 3) * 20}%`,
      ...
      <div className="max-w-7xl w-full px-6 relative z-10 flex flex-col items-center">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-500 text-sm font-medium max-w-lg mx-auto leading-relaxed">
            Witness the assembly of your autonomous future. A cinematic
            traversal through the five pillars of AI activation.
          </p>
        </div>
            ...
            <AnimatePresence initial={false} mode="popLayout">
              ...
              >
                ...
                <div className="absolute inset-0 bg-white/[0.01] rounded-[2rem] md:rounded-[4rem] border border-white/10 backdrop-blur-3xl overflow-hidden flex flex-col md:flex-row shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)]">
                  <div
                    className={cn(
                      "w-full md:w-1/2 relative flex items-center justify-center bg-gradient-to-br transition-all duration-1000 overflow-hidden py-12 md:py-0 min-h-[200px] md:min-h-0",
                      currentStep.color,
                    )}
                  >
                    ...
                    <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10 flex items-center gap-4">
                      <span className="text-3xl md:text-[40px] font-black text-white/10">
                        0{state.active}
                      </span>
                  ...
                  <div className="w-full md:w-1/2 p-6 md:p-16 flex flex-col justify-center relative bg-black/20">
                    ...
                    >
                      <span
                        className={cn(
                          "inline-block mb-4 px-3 py-1 rounded-full border border-white/5 bg-white/5 text-xs font-bold uppercase tracking-widest",
                          currentStep.accent,
                        )}
                      >
                        {currentStep.label}
                          ...
                          >
                            <div
                              className={cn(
                                ...
                                currentStep.accent.replace("text-", "bg-"),
                              )}
                            />
                            <span className="text-gray-300 text-xs font-black uppercase tracking-[0.2em]">
                              {detail}
                            </span>
                          </div>
                        ))}
                ...
                <div className="flex flex-col items-center gap-2 relative z-10 transition-all duration-300">
                  <div
                    className={cn(
                      ...
                      state.active === step.id
                        ? "scale-110 " + step.accent
                        : "text-gray-600 scale-90 group-hover:text-gray-400",
                    )}
                  >
                    {step.icon}
                  </div>
