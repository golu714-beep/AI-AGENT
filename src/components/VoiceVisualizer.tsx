    ...
    <div className="relative w-56 h-56 flex items-center justify-center">
      ...
      {[0, 90, 180, 270].map((deg) => (
        ...
        >
          ...
          <span className="absolute top-4 text-xs font-mono text-gray-700">
            {deg}°
          </span>
        </div>
      ))}
      ...
      {[...Array(3)].map((_, i) => (
        <motion.div
          ...
          className={cn(
            ...
            i === 1
              ? "inset-4 border-primary/30"
              : i === 2
                ? "inset-12 border-white/10"
                : "inset-2 border-white/5",
          )}
        />
      ))}
      ...
      <div className="relative w-36 h-36 rounded-2xl border border-white/10 bg-[#0A0A0B]/80 flex items-center justify-center overflow-hidden shadow-[inset_0_0_40px_rgba(0,0,0,0.8)]">
        ...
        {isPlaying ? (
          ...
          <div className="flex flex-col items-center gap-3 z-10">
            ...
            <span className="text-[7px] font-mono text-gray-500 tracking-widest uppercase">
              Target_Idle
            </span>
          </div>
        )}
      ...
      <div className="absolute top-0 left-0 text-xs font-mono text-primary/40 uppercase">
        X_Pos: 8.42
      </div>
      <div className="absolute bottom-0 right-0 text-xs font-mono text-primary/40 uppercase">
        Y_Pos: 12.09
      </div>
    </div>
  );
      ...
      <div className="flex items-center justify-between z-10">
        ...
        <div className="flex gap-2">
          <button
            onClick={() =>
              setActiveIndex(
                (prev) => (prev - 1 + voices.length) % voices.length,
              )
            }
            className="p-2 border border-white/10 bg-white/[0.02] hover:bg-white/5 text-gray-500 transition-all"
          >
            <SkipBack className="w-4 h-4" />
          </button>
          <button
            onClick={() => setActiveIndex((prev) => (prev + 1) % voices.length)}
            className="p-2 border border-white/10 bg-white/[0.02] hover:bg-white/5 text-gray-500 transition-all"
          >
        ...
        <div className="w-full md:w-64 space-y-6">
          <div className="relative p-7 border border-white/10 bg-[#0A0A0B] overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <h4 className="text-3xl font-black text-white tracking-tighter mb-1 uppercase italic">
                {currentVoice.name}
              </h4>
              ...
              <div className="space-y-4">
                {[
                  ...
                  {
                    label: "Model_Trust",
                    value: `${currentVoice.confidence}%`,
                  },
                ].map((item, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-end border-b border-white/[0.03] pb-1"
                  >
                    <span className="text-[7px] font-mono text-gray-600 uppercase">
                      {item.label}
                    </span>
      ...
      <div className="flex justify-between items-end relative z-10 pt-6 border-t border-white/5">
        <div className="grid grid-cols-3 gap-8">
          <div className="flex flex-col">
            <span className="text-[6px] font-mono text-gray-500 uppercase tracking-widest">
              Protocol
            </span>
